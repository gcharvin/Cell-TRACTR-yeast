...
class DETRSegmBase(nn.Module):
    def __init__(self, freeze_detr=False,return_intermediate_masks=False):
        if freeze_detr:
        ...
        self.mask_head = MaskHeadSmallConv(
            dim = self.hidden_dim + nheads,
            fpn_dims =  self.fpn_channels,
            context_dim = 256,
            return_intermediate=self.return_intermediate_masks)

    def forward(self, samples: NestedTensor, targets: list = None, prev_features=None):
        out, targets, features, memory, hs, prev_out = super().forward(samples, targets, prev_features=prev_features)
        ...
        if isinstance(memory, list):
            ...
            fpns = [features[len(features) - i - 1].tensors for i in range(1,len(features))]
            memory = memory[-1]
        ...
        # images = torch.stack([target['image'] for target in targets],axis=0)

        seg_masks = self.mask_head(src, bbox_mask, fpns)

        for m in range(len(seg_masks)):
            seg_masks[m] = seg_masks[m].view(
                batch_size, hs.shape[2], 2, seg_masks[m].shape[-2], seg_masks[m].shape[-1])

        # out["pred_masks"] = outputs_seg_masks
        out["pred_masks"] = seg_masks[-1]

        if self.return_intermediate_masks:
            for i,seg_mask in seg_masks:
                if i == len(seg_masks) - 1:
                    continue
                out["aux_outputs"][i]['pred_mask'] = seg_mask
...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, context_dim,return_intermediate=False):
        super().__init__()
        ...
        if return_intermediate:
            self.out_2 = torch.nn.Conv2d(inter_dims[1],2,3,padding=1)
            self.out_3 = torch.nn.Conv2d(inter_dims[2],2,3,padding=1)
            self.out_4 = torch.nn.Conv2d(inter_dims[3],2,3,padding=1)
            self.out_4 = torch.nn.Conv2d(inter_dims[4],2,3,padding=1)
